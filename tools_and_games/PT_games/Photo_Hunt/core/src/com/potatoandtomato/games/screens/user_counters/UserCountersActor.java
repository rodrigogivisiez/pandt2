...
public class UserCountersActor extends Table {
    ...
    public void populate(){
        ...
        for (int i = 0; i < 8 ; i++) {
            ...
            this.add(userTable).uniform().padRight(10);
            if(i == 3) this.row();
        }
    ...
    public void updateSorting(ArrayList<String> userIdsSortedByRecord){

        HashMap<Integer, Vector2> originalPositions = new HashMap();
        for(int i = 0; i < this.getCells().size; i++){
            Actor actor = this.getCells().get(i).getActor();
        ...
        for(int i =0; i < userIdsSortedByRecord.size(); i++){
            ...
            for(int q = 0; q < this.getCells().size; q++){
                if(this.getCells().get(q).getActor() == movingTable){
                    movingIndex = q;
                    break;
                }
            }
    ...
    public void setUserNameVisibility(String userId, boolean visible){
        Table userTable = userIdToTableMap.get(userId);
        Table counterTable = userTable.findActor("counterTable");
